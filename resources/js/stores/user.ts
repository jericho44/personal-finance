...
import type { IDataTableUser, ICreateUserPayload, IEditUserPayload, IUser } from '@/types/user'
...
export const useUserStore = defineStore('user', () => {
    // State
    const table = reactive<IDataTableUser>({
        column: [
            { text: 'NO', sortBy: 'id', sortColumn: true },
            { text: 'Nama', sortBy: 'name', sortColumn: true, class: 'w-300px' },
            { text: 'Username', sortBy: 'username', sortColumn: true, class: 'w-300px' },
            { text: 'Role', sortBy: 'role', sortColumn: false, class: 'text-center' },
    ...
    async function create(payload: ICreateUserPayload) {
    ...
    async function update(id: string | number, payload: IEditUserPayload) {
    ...
    async function resetPassword(id: string | number) {
        return api().put(`${prefix}${resource}/${id}/reset-password`)
    }
